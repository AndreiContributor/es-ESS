"""Validated runtime-version baseline for es-ESS.

The Wattpilot control protocol and Venus OS integration surfaces are not a
stable public API.  Keep the versions validated on the reference installation
in one auditable place and fail closed when a running system differs.
...
def require_validated_venus_os(actual=None, paths=None):
    ...
    if not versions_match(detected, VALIDATED_VENUS_OS_VERSION):
        raise CompatibilityError(
            "Unsupported Venus OS version {0}; es-ESS is validated only on {1}. "
