...
Deployments may include an optional interlocked maintenance bypass around the
Victron AC path, but the repository assumes no particular physical layout.
Grid-exchange metering, AC-coupled PV placement, the boundary governed by
`SiteMaxCurrent`, and downstream protective devices must be established from
the installation's electrical design. Cerbo GX and battery connections are
supervisory/DC boundaries, not additional AC series elements.
...
the switch position. An optional, gated backlog investigation covers explicit
bypass indication and fallback semantics; the current runtime must not claim
an automatic return to standalone Wattpilot use.
...
| --- | --- | --- | --- | --- |
| `SolarOverheadDistributor` | `SolarOverheadDistributor.py` | `[SolarOverheadDistributor]`, `[Common]`, `HttpConsumer:*`, `MqttConsumer:*` | Calculates available PV surplus, battery reservation, and per-consumer allowances. | Reads Victron system grid and battery D-Bus paths, publishes settings and fake battery reservation D-Bus services, and subscribes/publishes under `es-ESS/SolarOverheadDistributor/Requests/...`. Diagnostics label calculated raw overhead separately from allocated consumer allowances; neither is a device command. Scripted consumers retain minimum/step allocation. Explicitly configured HTTP/MQTT NPC loads are not discovered automatically and receive their complete `Request` or zero before bounded `[Common] HttpRequestTimeout` control work runs outside the shared consumer-map lock. |
...
| `FroniusWattpilot` | `FroniusWattpilot.py` | `[FroniusWattpilot]`, `[Shelly3EMSiteCurrent]` | Integrates and controls a Fronius Wattpilot EV charger. | Owns Victron EV-charger D-Bus paths, including session energy/time compatibility paths, site-current diagnostics, and the configured vehicle phase-capability observation; Wattpilot WebSocket commands through `Wattpilot.py`; SolarOverheadDistributor requests; grid telemetry safety checks; and mandatory physical L1/L2/L3 whole-site current protection through an explicitly selected `VenusSystem` or `Shelly3EMGen3` provider plus `WattpilotSiteCurrentDecisions.py`. `VehiclePhaseCapability=OnePhaseOnly` constrains only Auto/Eco: it suppresses phase-up and three-phase demand, caps direct current requests to one-phase capacity, rejects three-phase dispatch at the final boundary, and safely reconciles already-active three-phase telemetry. Manual remains user-controlled. The Shelly provider is a private read-only input and never registers as a Victron grid meter. Its controller-owned poll wrapper immediately withdraws only the Wattpilot distributor request on selected-source failure and logs sanitized failure/recovery transitions; charger commands remain on the normal controller cycle. An opt-in connection-only grace can retain the last complete still-fresh sample briefly for an existing charge while starts, increases, and phase-up stay blocked; other failures and grace expiry fail closed. Positive demand waits for a complete successful poll and normal site-current recovery. The service also owns read-only native `fup`/`ful` command-authority enforcement, runtime-status publication, and shutdown behavior. It supplies confirmed state to the command-free `WattpilotSessionStatistics.py` observer and logs transition records plus at most one APP_DEBUG checkpoint per connected minute; the observer has no D-Bus, MQTT, transport, or command access. The underlying `Wattpilot.py` client timestamps `nrg` current telemetry and owns a single worker reconnect loop for WebSocket outages. With `HibernateMode=true`, es-ESS intentionally disconnects while no EV is present; VRM mode changes are unsupported while disconnected, and Scheduled is only a best-effort status probe. See `docs/wattpilot-architecture.md` before changing it. |
...
The EV-charger service also publishes the read-only configured
`/VehiclePhaseCapability` value on D-Bus and retained main MQTT. `Automatic`
allows the normal one-/three-phase controller policy. `OnePhaseOnly` is an
operator-selected Auto/Eco constraint and is not inferred from charging
telemetry or treated as a persistent vehicle identity.
...
Structured Wattpilot session statistics are log evidence rather than another
...
`/Calculations/OverheadAvailable` is the raw calculated allocation input.
`Requests/{consumer}/Allowance` is that consumer's allocated result. Neither is
a device command. When the selected asynchronous Wattpilot site-current source
fails, only the Wattpilot `Request` is withdrawn; other consumers and the raw
