...
| [FroniusWattpilot]  | MinPhaseSwitchSeconds  | Shared continuous-condition timer for both 1-to-3 and 3-to-1 changes, and minimum interval between phase commands. Its duration, with a 600-second floor, also sets the separate slow same-phase current-increase wait when assigned PV covers only the next ampere. During a 3-to-1 wait, only bounded battery assist or explicitly permitted grid fallback may hold the running charge. | Integer (seconds) | 600 |
