...
| [FroniusWattpilot] | SurplusDropGraceSeconds | Non-negative grace period before continuous low surplus resets the Auto/Eco start timer. It also preserves an active 1-to-3 phase candidate through a shorter-than-grace dip only while assigned allowance remains above the effective three-phase floor (the greater of the configured phase-down threshold and electrical minimum); the full phase-up threshold is still required at the command boundary. | Integer (seconds) | 20 |
...
- Both one-to-three and three-to-one phase changes use `MinPhaseSwitchSeconds` as their normal stability timer and minimum interval between phase commands. An active one-to-three candidate survives a shorter-than-`SurplusDropGraceSeconds` dip below the phase-up threshold only while fresh assigned allowance remains above the effective three-phase floor. A deeper or longer dip resets the candidate, and the full phase-up threshold must be present again before the phase command is sent.
