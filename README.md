...
| [FroniusWattpilot] | SiteCurrentRecoverySeconds | Non-negative continuous safe-headroom time before a stopped charge may restart. During a running same-phase charge, both safe headroom and fresh assigned PV must continuously cover the next ampere plus one additional allocation step for this interval; current then rises by only 1 A and rebuilds the interval before another increase. Reductions remain immediate. | Integer (seconds) | 30 |
...
- After headroom recovers, it must remain safe for `SiteCurrentRecoverySeconds`. A running same-phase increase also requires fresh assigned PV to continuously cover the next ampere plus one additional live allocation step for that same interval and across distinct allowance updates. The controller then rises by only 1 A and rebuilds the interval before another increase. This dynamic one-step reserve absorbs the normal allowance feedback change after measured EV demand rises; it adds no configuration parameter. It can keep a running same-phase charge one ampere below the effective maximum, while explicit starts and phase transitions can still select their independently guarded target. Current reductions and safety stops remain immediate. No normal increase is issued while the connected EV reports no material charging power. A genuine current reduction resets recovery, while an unchanged safe active-current target preserves site recovery so the controller can restore its positive distributor request instead of remaining at zero allowance. A stopped session still obeys `MinOnOffSeconds`, and a running one-phase session still needs `MinPhaseSwitchSeconds` before phase-up. A new stopped session can start directly on three phases once the normal start and site-recovery conditions are both satisfied.
...
SolarOverheadDistributor diagnostics distinguish the calculated raw overhead
from each consumer's allocated allowance. Neither value is itself a charger
command. For Wattpilot, only the normal Auto/Eco controller may turn an eligible
fresh allowance into guarded phase, current, or start commands.

Wattpilot firmware `42.5` accepts whole-ampere current limits. Strict PV-only
control therefore normally leaves less than one active-phase canonical step as
...
`/Calculations/OverheadAvailable` and its matching MQTT/service diagnostics are
the calculated raw allocation input. A consumer `Allowance` is the distributor's
allocated share after request, minimum, step, priority, consumption, and battery
reservation processing. Raw overhead and allowance are diagnostic/allocation
values, not device commands; each consumer remains responsible for its own
