...
| [FroniusWattpilot] | SiteCurrentRecoverySeconds | Non-negative continuous safe-headroom time before a stopped charge may restart. During a running same-phase charge, both safe headroom and fresh assigned PV must continuously support a higher current for this interval; current then rises by 1 A and rebuilds the interval before another increase. Reductions remain immediate. | Integer (seconds) | 30 |
...
- After headroom recovers, it must remain safe for `SiteCurrentRecoverySeconds`. A running same-phase increase also requires fresh assigned PV to continuously support at least the next ampere for that same interval and across distinct allowance updates. The controller then rises by only 1 A and rebuilds the interval before another increase. A brief higher allocation therefore cannot cause a one-cycle increase followed by an immediate reversal. Current reductions and safety stops remain immediate. No normal increase is issued while the connected EV reports no material charging power; explicit start and phase-transition transactions retain their existing guards. A genuine current reduction resets recovery, while an unchanged safe active-current target preserves site recovery so the controller can restore its positive distributor request instead of remaining at zero allowance. A stopped session still obeys `MinOnOffSeconds`, and a running one-phase session still needs `MinPhaseSwitchSeconds` before phase-up. A new stopped session can start directly on three phases once the normal start and site-recovery conditions are both satisfied.
