...
class RuntimeCompatibilityTests(unittest.TestCase):
    def test_normalization_accepts_optional_v_prefix(self):
        self.assertEqual(compatibility.normalize_version("v3.75"), "3.75")
        self.assertEqual(compatibility.normalize_version(" 3.75 "), "3.75")
        self.assertTrue(compatibility.versions_match("v3.75", "3.75"))

    def test_qualifier_does_not_match_clean_release(self):
        self.assertFalse(compatibility.versions_match("v3.75~1", "v3.75"))
        self.assertFalse(compatibility.versions_match("v3.75-beta", "v3.75"))

    def test_missing_version_never_matches(self):
        self.assertFalse(compatibility.versions_match(None, "v3.75"))
        self.assertFalse(compatibility.versions_match("", "v3.75"))
    ...
    def test_other_or_missing_venus_version_fails_closed(self):
        for actual in ("v3.74", "v3.75~1", "v3.76", None):
            with self.subTest(actual=actual):
