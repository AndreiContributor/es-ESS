...
class RuntimeCompatibilityTests(unittest.TestCase):
    def test_preferred_and_rollback_releases_are_explicit(self):
        self.assertEqual("v3.79", compatibility.VALIDATED_VENUS_OS_VERSION)
        self.assertEqual(
            ("v3.75", "v3.79"),
    ...
    def test_normalization_accepts_optional_v_prefix(self):
        self.assertEqual(compatibility.normalize_version("v3.79"), "3.79")
        self.assertEqual(compatibility.normalize_version(" 3.79 "), "3.79")
        self.assertTrue(compatibility.versions_match("v3.79", "3.79"))

    def test_qualifier_does_not_match_clean_release(self):
        self.assertFalse(compatibility.versions_match("v3.79~1", "v3.79"))
        self.assertFalse(compatibility.versions_match("v3.79-beta", "v3.79"))

    def test_missing_version_never_matches(self):
        self.assertFalse(compatibility.versions_match(None, "v3.79"))
        self.assertFalse(compatibility.versions_match("", "v3.79"))
    ...
    def test_approved_venus_versions_are_accepted(self):
        for actual in ("3.75", "v3.75", "3.79", "v3.79"):
    ...
    def test_failure_lists_every_approved_version(self):
        with self.assertRaises(compatibility.CompatibilityError) as raised:
            compatibility.require_validated_venus_os(actual="v3.80")
        self.assertIn("v3.75", str(raised.exception))
        self.assertIn("v3.79", str(raised.exception))
